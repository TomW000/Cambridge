from setup import neurotransmitters, model, device, feat_dim, resize_size, dataset_path, tqdm, torch, np, os, h5py
...
if __name__=='__main__':  # Fixed: Added double underscores
    ...
    #good_idx = [17,316,617,924,1225,1512]#
    ...
    good_files = [files[idx] for idx in good_idx] # len = 30
    datasets = [good_files[i:i+5] for i in range(0, len(good_idx), 5)] # list of lists of files
    good_labels = [labs[idx] for idx in good_idx]
    ...
    mean_ref = np.vstack([np.mean(list) for list in latent_list])
